 ...
 * @param to 目标表。
 */
function convert(value: string, from: string, to: string) {
    let result = [];
    for (let i = 0; i < value.length; i++) {
        let c = value.charAt(i);
        let p = from.indexOf(c);
        if (p >= 0) c = to.charAt(p);
        result.push(c);
    }
    return result.join('');
}
...
export function toSimpleChinese(value: string) {
    return convert(value, tradionalChineseDict, simpleChineseDict);
}
...
export function toTradionalChinese(value: string) {
    return convert(value, simpleChineseDict, tradionalChineseDict);
}
